...
class PerformanceMetricsService
{
    ...
    public function aggregateDailyMetrics(Carbon $date)
    {
        ...
        foreach ($groups as $groupKey => $groupItems) {
            ...
            $p50 = array_sum(array_map(function ($item) {
                return $item['p50'] * $item['weight'];
            }, $allValues));
            $p75 = array_sum(array_map(function ($item) {
                return $item['p75'] * $item['weight'];
            }, $allValues));
            $p90 = array_sum(array_map(function ($item) {
                return $item['p90'] * $item['weight'];
            }, $allValues));
            $p95 = array_sum(array_map(function ($item) {
                return $item['p95'] * $item['weight'];
            }, $allValues));
            $p99 = array_sum(array_map(function ($item) {
                return $item['p99'] * $item['weight'];
            }, $allValues));
            $avg = array_sum(array_map(function ($item) {
                return $item['avg'] * $item['weight'];
            }, $allValues));
